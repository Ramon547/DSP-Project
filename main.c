...
/* USER CODE BEGIN PV */
const uint32_t UPTO = 0x0000000F;

uint32_t audio[2] = {0};
uint32_t buffer[SIZE_DECL][2] = {0};
uint32_t past[0x00003FFF][2] = {0};

uint8_t read[0x00003FFF][4] = {0};
uint8_t write[SIZE_DECL << 1][4];
uint32_t address = 0;
uint8_t address_8bit[3] = {0};
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  Convert_Address(address & 0x0001C000, address_8bit);
	  EEPROM_Read_Bulk(&hspi1, (address & 0x00020000) >> 11, &read[0][0], address & 0x00003FFF, address_8bit);
	  PJ_W32(&read[0][0], &past[0][0], address & 0x00003FFF);
		switch(state & 0x03){
...
/* USER CODE BEGIN 4 */
//Function to initialize codec
//No inputs, no outputs
void Codec_Init(void){

	//For loop to write necessary values to correct registers
	for(uint8_t i = 0; i < INIT_VALS; i++){
		//Send data to register via I2C
		HAL_I2C_Mem_Write(&hi2c1, ADDRESS, REGS[i], 1, VALS + i, 1, HAL_MAX_DELAY);
	}

}

//Function to initialize EEPROMs
//No inputs, no outputs
void EEPROM_Init(void){

	//Write status to bank 0
	EEPROM_Write_SR_BC(&hspi1, 0x00, &STATUS);

	//Write status to bank 1
	EEPROM_Write_SR_BC(&hspi1, 0x04, &STATUS);

}

//Function to turn on PWM output
//No inputs, no outputs
void PWM_Init(void){
